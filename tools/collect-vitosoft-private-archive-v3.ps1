...
function Join-ProcessArgs {
  param([object[]]$ArgumentItems)
  if ($null -eq $ArgumentItems -or $ArgumentItems.Count -eq 0) { return "" }
  return (($ArgumentItems | ForEach-Object { Quote-ProcessArg ([string]$_) }) -join " ")
}
...
function New-ToolEncodedCommand {
  param([object]$Task)
...
function Start-ToolTask {
  param([object]$Task)
  if ($Task.StdOut) { New-Item -ItemType Directory -Path (Split-Path -Parent $Task.StdOut) -Force | Out-Null }
  if ($Task.StdErr) { New-Item -ItemType Directory -Path (Split-Path -Parent $Task.StdErr) -Force | Out-Null }
  $encoded = New-ToolEncodedCommand -Task $Task
  $argLine = "-NoProfile -NonInteractive -ExecutionPolicy Bypass -EncodedCommand " + $encoded
  return Start-Process -FilePath "powershell.exe" -ArgumentList $argLine -NoNewWindow -PassThru
}

function Test-ExpectedToolFailure {
  param([string]$Name,[object]$Task)
  if ($Name -ne "ildasm") { return $false }
  if (-not $Task.StdErr -or -not (Test-Path -LiteralPath $Task.StdErr -PathType Leaf)) { return $false }
  try { return [bool](Select-String -LiteralPath $Task.StdErr -Pattern "Protected module -- cannot disassemble" -SimpleMatch -Quiet) } catch { return $false }
}
...
function Invoke-ThrottledProcessTasks {
  ...
  $sw=[Diagnostics.Stopwatch]::StartNew(); $done=0; $failed=0; $blocked=0

  # First task is a real probe. Task lists are ordered so vsmInterfaceCommon.dll
  # is preferred where available. A systemic runner error therefore stops the
  # whole group before dozens of identical failures are spawned.
  $probe=$Tasks[0]
  try {
    $p=Start-ToolTask -Task $probe; $p.WaitForExit(); $probeExit=$p.ExitCode; $p.Dispose()
  } catch {
    $probeExit=-1
    if ($probe.ErrorFile) { $_.Exception.ToString() | Set-Content -LiteralPath $probe.ErrorFile -Encoding UTF8 }
  }
  $done=1
  if ($probeExit -ne 0) {
    if (Test-ExpectedToolFailure -Name $Name -Task $probe) { $blocked=1 }
    else {
      $failed=1
      Write-Warning ("{0} probe failed with exit code {1}; skipping remaining {2} tasks." -f $Name,$probeExit,($Tasks.Count-1))
      if ($probe.StdErr -and (Test-Path -LiteralPath $probe.StdErr)) {
        Get-Content -LiteralPath $probe.StdErr -TotalCount 12 -ErrorAction SilentlyContinue | ForEach-Object { Write-Host ("  " + $_) -ForegroundColor Yellow }
      }
      $sw.Stop()
      [void]$phaseTimings.Add([pscustomobject]@{Phase=$Name;Seconds=[math]::Round($sw.Elapsed.TotalSeconds,3);Tasks=$Tasks.Count;Parallelism=$Throttle;Failed=$failed;Blocked=$blocked})
      return
    }
  }

  $queue=New-Object System.Collections.Queue
  for($i=1;$i -lt $Tasks.Count;$i++){ $queue.Enqueue($Tasks[$i]) }
  $running=New-Object System.Collections.ArrayList
  while($queue.Count -gt 0 -or $running.Count -gt 0){
    while($queue.Count -gt 0 -and $running.Count -lt $Throttle){
      $task=$queue.Dequeue()
      try { $p=Start-ToolTask -Task $task; [void]$running.Add([pscustomobject]@{Process=$p;Task=$task}) }
      catch { $failed++; $done++; if($task.ErrorFile){$_.Exception.ToString()|Set-Content -LiteralPath $task.ErrorFile -Encoding UTF8} }
    }
    foreach($entry in @($running)){
      if(-not $entry.Process.HasExited){continue}
      $exitCode=$entry.Process.ExitCode; try{$entry.Process.Dispose()}catch{}; [void]$running.Remove($entry); $done++
      if($exitCode -ne 0){
        if(Test-ExpectedToolFailure -Name $Name -Task $entry.Task){ $blocked++ }
        else { $failed++; if($entry.Task.ErrorFile){("ExitCode="+$exitCode)|Set-Content -LiteralPath $entry.Task.ErrorFile -Encoding UTF8} }
      }
      if(($done % 25)-eq 0 -or $done -eq $Tasks.Count){ Write-Host ("  {0}: {1}/{2} complete, failed={3}, blocked={4}" -f $Name,$done,$Tasks.Count,$failed,$blocked) }
    }
...
function Start-CollectorChild {
  ...
  $argsLine = Join-ProcessArgs -ArgumentItems $Arguments
  $started = Get-Date
...
function Find-ToolPath {
  ...
  switch ($Name.ToLowerInvariant()) {
    ...
    "ilspycmd.exe" {
      $userTool = Join-Path $env:USERPROFILE ".dotnet\tools\ilspycmd.exe"
      if (Test-Path -LiteralPath $userTool -PathType Leaf) { return $userTool }
    }
    "7z.exe" {
...
function Write-PrerequisiteReport {
  ...
  @(
    ...
    [pscustomobject]@{Name="dotnet";Required=$false;Available=[bool]$State.dotnet;Path=$State.dotnet;AutoInstall=$true;Purpose=".NET 10 SDK for ILSpyCmd"}
    [pscustomobject]@{Name="ilspycmd";Required=$false;Available=[bool]$State.ilspycmd;Path=$State.ilspycmd;AutoInstall=$true;Purpose="Decompiler fallback for protected managed assemblies"}
...
function Install-ResearchPrerequisites {
  ...
  try {
    ...
    # ILSpyCmd is the fallback for assemblies protected against ILDASM.
    ...
    if (-not $ilspy) {
      ...
      if ($dotnet) {
        try {
          ...
          & $dotnet.Source tool install --global ilspycmd --version 11.0.0.9375 2>&1 |
            ForEach-Object { $log.WriteLine([string]$_) }
          if ($LASTEXITCODE -ne 0) {
            & $dotnet.Source tool update --global ilspycmd --version 11.0.0.9375 2>&1 |
              ForEach-Object { $log.WriteLine([string]$_) }
          }
          $env:PATH = $env:PATH + ";" + (Join-Path $env:USERPROFILE ".dotnet\tools")
        } catch { $log.WriteLine("ILSpyCmd install error: " + $_.Exception.ToString()) }
      }
    }
...
if (-not $Sequential -and $Parallelism -gt 1) {
  ...
  if (-not $SkipDeep) {
    ...
    $phase.Stop()
    [void]$phaseTimings.Add([pscustomobject]@{Phase="deep-derived-collector";Seconds=[math]::Round($phase.Elapsed.TotalSeconds,3);Tasks=1;Parallelism=1;Failed=$(if($deepExit -eq 0){0}else{1});Blocked=0})
    if ($deepExit -ne 0) {
...
if (-not $SkipToolDumps) {
  ...
  $ilspyPath = Find-ToolPath "ilspycmd.exe"
  ...
  $managedCandidates = @($peFiles | Where-Object { $_.Extension.ToLowerInvariant() -in @(".exe",".dll") } | Sort-Object @{Expression={if($_.Name -ieq "vsmInterfaceCommon.dll"){0}else{1}}}, FullName)
  ...
  if ($ilspyPath) {
    $ilspyDir = Join-Path $toolsDir "ilspy-csharp"
    New-Item -ItemType Directory -Path $ilspyDir -Force | Out-Null
    $tasks = @(
      foreach ($f in $managedFiles) {
        $out = Join-Path $ilspyDir ((Safe-Name (RelPath $Root $f.FullName)) + ".cs.txt")
        [pscustomobject]@{
          FilePath=$ilspyPath
          Arguments=@("--disable-updatecheck","--ignore-decompilation-errors",$f.FullName)
          StdOut=$out
          StdErr=($out + ".stderr.txt")
          ErrorFile=($out + ".error.txt")
        }
    ...
    Invoke-ThrottledProcessTasks -Name "ilspy-csharp" -Tasks $tasks -Throttle $Parallelism
  }
  ...
  if ($dumpbinPath) {
    ...
    $tasks = @(
      foreach ($f in @($peFiles | Where-Object { $_.Extension.ToLowerInvariant() -in @(".exe",".dll",".sys") } | Sort-Object @{Expression={if($_.Name -ieq "vsmInterfaceCommon.dll"){0}else{1}}}, FullName)) {
