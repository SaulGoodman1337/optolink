...
function Test-ExpectedToolFailure {
  ...
  try {
    return [bool](Select-String -LiteralPath $Task.StdErr -Pattern "Protected module -- cannot disassemble" -SimpleMatch -Quiet)
  } catch {
    return $false
  }
}
...
function Invoke-ThrottledProcessTasks {
  ...
  # Intentionally sequential on Windows PowerShell 5.1.
  # Direct invocation is the same execution path verified manually for
  ...
  foreach ($task in $Tasks) {
    ...
    try {
      ...
      if ($exitCode -ne 0) {
        if (Test-ExpectedToolFailure -Name $Name -Task $task) {
          ...
          $failed++
          if ($task.ErrorFile) {
            ("ExitCode=" + $exitCode) | Set-Content -LiteralPath $task.ErrorFile -Encoding UTF8
          }
        }
      }
    } catch {
    ...
    if (($done % 25) -eq 0 -or $done -eq $Tasks.Count) {
      Write-Host ("  {0}: {1}/{2} complete, failed={3}, blocked={4}" -f $Name,$done,$Tasks.Count,$failed,$blocked)
    }
  }
...
Write-Host ("Deep/SQL workers: {0}  |  Robocopy threads: {1}  |  Tool dumps: direct/sequential" -f $Parallelism,$CopyThreads)
Write-Host ""
