...
| GFA transport | Mixed VS1 hardware compatibility is confirmed. The guarded stock-splitter permanent-VS1 smoke gate is now prepared with write ingress disabled, MQTT read-topic coverage checks, stable MainPID/journal checks and byte-exact settings restore. | Run the prepared 30 s stock-splitter gate. After a PASS, add a narrow structured GFA path under the splitter's single serial owner. |
