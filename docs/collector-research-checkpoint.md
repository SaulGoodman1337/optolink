...
| GFA transport | Independent and persistent-session GFA reads are live-confirmed. Upstream source supports permanent VS1 Virtual_READ/WRITE (F7/F4), but its structured VS1 request path lacks GFA_READ support. | Run the prepared read-only mixed F7/6B compatibility probe before any `vs1protocol=True` production change. If it passes, add a narrow structured GFA path under the splitter's single serial owner. |
...
Persistent-session runtime access and startup capture are confirmed. P87 bit 1 precedes P09 plateau release, but remains semantically unnamed; public WB2A/VSKO material makes an end-of-start/stabilization marker more plausible than first flame recognition without proving that mapping. The current next step is the read-only [mixed VS1 Virtual/GFA compatibility probe](vs1-mixed-gfa-integration.md). It compares stable P300 values against F7 reads while interleaving P80/P06/P09/P87 6B reads in one VS1 session, then verifies P300 again. Do not switch production to `vs1protocol=True` until that passes. The updater hard-resets the upstream checkout, so any later splitter patch must be applied reproducibly by the repo updater/installer or accepted upstream.
