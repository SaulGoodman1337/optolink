...
| Flame stabilization / startup plateau | Self-triggered run correlates P87 `60->62` (bit 1 set) with release of P09 raw `93` / 57.6534% plateau. Static review found no P87 bit table in the inspected archive/public export. | Run the prepared P87/P09 hi-res follow-up to narrow transition ordering while keeping 150-ms pacing. Do not name bit 1 as flame/stabilization; no start-safety writes. |
...
Persistent-session runtime access and startup capture are confirmed. The guarded 37 C run tied P87 `60->62` tightly to modulation-plateau release, but static source review still cannot name bit 1. The prepared [P87/P09 high-resolution probe](gfa-p87-p09-hires.md) samples two pairs per P80-guarded block with alternating order and unchanged 150-ms pacing. After that timing check, continue coordinated VS1/P300 ownership. Do not promote GFA values to production HA sensors until the remaining FF quality issue and integration/recovery policy are resolved.
