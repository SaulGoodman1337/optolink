...
| Flame stabilization / startup plateau | Self-triggered run now correlates P87 `60->62` (bit 1 set) with release of P09 raw `93` / 57.6534% plateau in the same sequential round. First P84=`06` to first P87=`62` = 10.148 s; P87=`60` to `62` = 9.866 s. P87 semantics remain unnamed. | Prioritize static/source interpretation of P87 and, only if needed, a higher-resolution read-only confirmation. Do not name bit 1 as flame/stabilization yet; no start-safety writes. |
...
Persistent-session runtime access and startup capture are confirmed. The [GFA status/startup probe](gfa-status-probe.md) first identified P87 bit 1 as a candidate; the guarded 37 C follow-up then captured the real controller-driven startup and tied P87 `60->62` tightly to the modulation-plateau release. The exact P87 bit meaning remains unknown. Next continue static/source interpretation and coordinated VS1/P300 ownership. Do not promote GFA values to production HA sensors until the remaining FF quality issue and integration/recovery policy are resolved.
