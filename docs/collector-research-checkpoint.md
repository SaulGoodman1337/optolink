...
> Historical checkpoint. For current priorities and TODOs use [research-plan-2026-09-24.md](research-plan-2026-09-24.md). Later production GFA integration, timing and v6 archival results supersede the old "next action" wording here, while the measurements and corrections remain valid.
...
| GFA transport | Permanent VS1, splitter-owned structured read-only GFA access and HA publication are hardware-verified active. Production timing is global 25 ms / GFA first 25 ms / retry 150 ms. | No further timing benchmark is planned unless runtime telemetry shows repeated retry failure, OL Error or restart. Continue semantic research, not transport re-validation. |
| Flame stabilization / startup plateau | Hi-res run resolves ordering: P87 bit 1 (`60->62`) is set before P09 leaves raw `93` / 57.6534%, with at least 257 ms between transition windows. WB2A/VSKO documentation separates early flame ionization (~2-3 s) from a 10-s ionization-monitored start phase; no P87 bit table exists in the inspected metadata. | Treat P87.b1 only as an unnamed marker/precursor near the end of the supervised start/stabilization interval. No faster polling needed; no start-safety writes. |
...
| Software identities | P80=0x20 and main-regulation raw 0x0103 are measured; official release-name mapping remains unknown. | Read GFA P81-P83 next and keep controller, burner, panel, pump, profile and coding-card identities distinct. |
...
| Coding plug | Two spare chip1 24C04-class dumps are repeatable; the board has two physically distinct EEPROMs and separate GWG/GFA software views. No side/domain assignment or write recovery is proven. | Capture both sides with explicit f01/f02 labels, then correlate active-plug GWG fields and read-only P90/P100-P108. |
...
Persistent-session runtime access and startup capture are confirmed. P87 bit 1 precedes P09 plateau release but remains semantically unnamed. The [mixed VS1 Virtual/GFA compatibility probe](vs1-mixed-gfa-integration.md) has now passed on hardware: all selected stable F7 values matched P300 before/after while 6B GFA reads ran in the same session. The next gate is the prepared [stock splitter permanent-VS1 smoke test](vs1-stock-splitter-smoke.md): existing HA read poll list, 30 s temporary VS1, MQTT/TCP write ingress disabled, Party stopped, all enabled cycle-0 MQTT topics required, and byte-exact settings restore. Only after that passes should the structured GFA polling patch be activated. The updater hard-resets the upstream checkout, so any later patch must be applied reproducibly by the repo updater/installer or accepted upstream.
