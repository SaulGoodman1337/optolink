...
| GFA transport | Mixed VS1 hardware compatibility is now confirmed: stable values matched P300->F7->P300 while P80/P06/P09/P87 6B reads were interleaved in one persistent session; P80 stayed 0x20 and no FF occurred in the bounded test. | Next validate the stock splitter + current HA read poll list in temporary permanent-VS1 mode with write ingress disabled. After that, add a narrow structured GFA path under the splitter's single serial owner. |
...
Persistent-session runtime access and startup capture are confirmed. P87 bit 1 precedes P09 plateau release but remains semantically unnamed. The [mixed VS1 Virtual/GFA compatibility probe](vs1-mixed-gfa-integration.md) has now passed on hardware: all selected stable F7 values matched P300 before/after while 6B GFA reads ran in the same session. The next gate is the stock splitter itself using the existing HA read poll list in temporary permanent-VS1 mode with MQTT/TCP write ingress disabled and the party emulator stopped. Only after that passes should the structured GFA polling patch be activated. The updater hard-resets the upstream checkout, so any later patch must be applied reproducibly by the repo updater/installer or accepted upstream.
