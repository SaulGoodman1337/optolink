...
| Flame stabilization / startup plateau | Hi-res run resolves ordering: P87 bit 1 (`60->62`) is set before P09 leaves raw `93` / 57.6534%. Transition windows are non-overlapping with at least 257 ms separation. Static review still found no P87 bit table. | No faster polling needed for ordering. Continue source/semantic investigation and production-safe VS1 integration. Do not name bit 1 as flame/stabilization; no start-safety writes. |
...
Persistent-session runtime access and startup capture are confirmed. The guarded P87/P09 high-resolution run now proves temporal ordering for the observed controller values: P87 bit 1 is already set before P09 leaves the high-start plateau, with at least 257 ms between the non-overlapping transition windows. Static source review still cannot name bit 1, so this remains an unnamed precursor/marker rather than a vendor-defined flame/stabilization signal. Continue coordinated VS1/P300 ownership and semantics work; do not increase polling rate merely to refine an ordering that is already resolved. Do not promote GFA values to production HA sensors until the remaining FF quality issue and integration/recovery policy are resolved.
