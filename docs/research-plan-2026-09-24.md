...
**Status: PAUSED on 2026-09-26 pending regulation-MCU / firmware evidence from Issue #25.**

Canonical pause checkpoint:
[vitotrol-software-emulation-pause-checkpoint-2026-09-26.md](../config/optolink-splitter/research/vitotrol-software-emulation-pause-checkpoint-2026-09-26.md).

The software-only path has reached the exposed Vitosoft/Optolink boundary:

- A0=1 is proven to arm remote expectation and raises BC without a responding remote; it is not emulation.
- Exact VDensHO1 RPCs are exhausted; no KM-BUS/Vitotrol RX mailbox was recovered.
- Production KBus write families are source-negative for raw Vitotrol slave-frame injection.
- Ordinary Virtual_WRITE to the effective room value `0x0896` is rejected.
- `0x089C` accepts a write request but the effective status is immediately regenerated by internal controller logic.
- Hidden `0x7340..0x7344` state is writable; the Vitosoft F2/F3 encoding strongly matches the physical Vitotrol 200/300 ID families, but isolated `0x7342=0x74` does not create a valid participant or a BC by itself.
- Cross-profile NRF aliases are useful firmware-family architecture evidence, but multiple direct address collisions prove they are not a VDensHO1 write recipe.
- Local production restore is verified: `0x27A0=00`, `0x7342=00`, `0x0896=C800`, `0x089C=03`, `0x0A5C=00000000`, current alarm clear.

**Reopen P2 only when the MCU/firmware track produces a concrete discriminator**, preferably over Optolink:

- regulation firmware image suitable for handler cross-references;
- exact Optolink monitor/bank/page/internal-memory service;
- exact RX-mailbox or participant-state mechanism;
- exact handler table explaining `0x0896` reject vs. `0x089C` regeneration vs. `0x734x` write acceptance;
- identified KM-BUS RX / Vitotrol watchdog routine and its state variables.

Until then:

- no further blind NRF/KBus/XRAM writes;
- no A0-only repeats;
- keep the physical Vitotrol protocol reconstruction as the behavioral oracle;
- focus research effort on Issue #25 and firmware access through Optolink.
