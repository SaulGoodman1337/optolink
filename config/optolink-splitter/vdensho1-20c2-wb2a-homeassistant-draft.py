"""
Home Assistant / MQTT discovery draft for the verified boiler profile:

  Viessmann Vitodens 200-W WB2A
  VDensHO1 / device id 0x20C2
  software index 0x03

This file is intentionally NOT active and is not referenced by the installer.
It targets the current optolink-splitter Home Assistant integration based on
...
The complete enums for 0x2323 and 0x6773 come from the exact VDensHO1
Vitosoft-derived catalog. Values not named above as hardware-tested should
still be treated as source-documented but not yet write-tested on this unit.
"""
...
poll_list = {
    ...
    "domains": [
        ...
        # Exact catalog: 0..4. Hardware writes verified for 2 <-> 4.
        # -----------------------------------------------------------------
        {
            "domain": "select",
            "options": [
            ...
            "value_template": "{% set v = value | int(-1) %}{% if v == 0 %}Abschalt{% elif v == 1 %}Nur WW{% elif v == 2 %}Heizen + WW{% elif v == 3 %}Dauernd Reduziert{% elif v == 4 %}Dauernd Normal{% else %}Unbekannt{% endif %}",
            "optimistic": False,
            "poll": [
        ...
        # -----------------------------------------------------------------
        # Circulation interval. 0 and 7 are hardware-verified R/W.
        # Physical "boost from OFF baseline" test is still intentionally open.
        # -----------------------------------------------------------------
        {
            "domain": "select",
            "options": [
            ...
            "value_template": "{% set v = value | int(-1) %}{% if v == 0 %}Schaltuhr{% elif v == 1 %}1 pro Stunde{% elif v == 2 %}2 pro Stunde{% elif v == 3 %}3 pro Stunde{% elif v == 4 %}4 pro Stunde{% elif v == 5 %}5 pro Stunde{% elif v == 6 %}6 pro Stunde{% elif v == 7 %}EIN{% else %}Unbekannt{% endif %}",
            "optimistic": False,
            "poll": [
