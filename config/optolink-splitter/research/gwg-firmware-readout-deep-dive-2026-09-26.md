...
## Active continuation: complete public Wiki text-history wire inventory

A command-oriented scan was run over every reachable **textual** OpenV Wiki
Git blob instead of searching only filenames or keywords.

Coverage:

```text
text blobs scanned:        1348
unique explicit SEND forms:  12
```

After normalizing literal `SEND 01 ...` wire sequences, the entire reachable
text history contains only these concrete forms:
...
The last `01 04` form is the tail generated by interactive GWG test commands
after a dynamic one-byte address and is not a standalone function-code frame.

No explicit textual request using:
...
survives anywhere in the reachable Wiki text history.

The only concrete low-level physical-read examples are:
...
from `Datenauswertung-mit-RRDB.md`.

Crucially, that page explicitly states immediately before those examples:

> the `get` test queries are configured only for the GWG protocol, not for
> KW.

Therefore these `CB` examples are not evidence for V200KW2 / `2098`.
They are ordinary GWG examples consistent with the existing `2053` path.

Several historical `vcontrold.xml` blobs contain both `2098` device
metadata and low-level GWG macro definitions, but the complete literal-wire
inventory confirms the already documented configuration-boundary result:
global protocol definitions coexisting in one XML file do not establish that
those frames were transmitted to `2098`.

### Consequence

The reachable **public Wiki text history** is now effectively exhausted as a
source for a missing persisted 2098 low-level wire example:

- no 2098-specific `CB/C5/AE/9E/33/43/6E` request;
- no selector/page/bank preamble;
- no monitor-entry sequence;
- no high-address/copy-mailbox request;
- no VitoTest command script preserving such a sequence.

A missing request could still survive in an external forum quote, private
developer artifact, machine-local VitoTest command history, unmirrored
attachment or binary-only implementation. Further work should prioritize those
surfaces rather than repeat scans of the public Wiki text corpus.
